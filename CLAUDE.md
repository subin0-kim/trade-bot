...
- `apps/bot_scalper`: 주식 단타봇 스텁 (KIS 분봉 — 분봉 전략 게이트 통과 후 본격 개발)
- `apps/bot_scalper_coin`: **코인 단타봇 (드라이런 단계, 2026-08-02~)** — 2단 지정가 사다리(-3/-5%, 5분 재산정) + 근접도 라우터 + 매물대 목표 청산 + 적응형 타임아웃(bull360/off120) + 광폭 하락 매수중지 + 유의·주의 종목 제외(플래그 로깅). 1분 상주 루프, 별도 원장, `--profile`로 사이징 4종 (기본 high-risk 50%×2)
