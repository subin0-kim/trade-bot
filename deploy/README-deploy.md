...
**예산**: 코인봇의 `--budget`(기본 1,000만원)은 **원장 초기 시드** — 첫 실행 때 원장이
이 값으로 생성되므로 **가동 전에 원하는 값으로 확정**할 것 (이후 변경은 state 파일 수정).
이후 사이징은 원장 자산(현금+평가액) 비례의 **복리** (2026-07-31 백테스트 의미론과 정렬):
벌면 진입 금액도 비례해 커지고, 잃으면 줄어든다. 스윙봇은 코드 상수 `INITIAL_CASH`(100만원).
봇은 계좌 잔고를 조회하지 않고 원장 자산만 쓰므로, 계좌의 기존 보유 자산·예수금은 불가침이다.
