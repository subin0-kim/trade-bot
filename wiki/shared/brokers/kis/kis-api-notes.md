...
## 주문 체결 확인 (TTTC0081R/VTTC0081R) — 모의서버 지원 (2026-07-28)

- 주식일별주문체결조회로 ODNO 필터 → tot_ccld_qty(총체결수량)/tot_ccld_amt(총체결금액),
  평균체결가 = 금액/수량. **휴장일 API와 달리 모의서버(VTTC0081R) 지원.**
- 응답 odno는 자리수 패딩 가능 — lstrip("0") 비교 필요.
- 라이브 주문 흐름: place_order → wait_fill(폴링 20초) → 실체결 수량·평균가로 원장 기록.
