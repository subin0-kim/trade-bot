...
사이징 프로파일 (--profile):
  ...
  uv run bot-scalper-coin --once            # 1사이클 dry-run (검증용)
  uv run bot-scalper-coin                   # 상주 루프 dry-run (1분 주기)
  uv run bot-scalper-coin --live --yes      # 실주문 (무인 승인)
"""
...
logger = logging.getLogger("bot_scalper_coin")

BOT_NAME = "bot_scalper_coin"
DATA_DIR = Path("data")
